...
function get_scrape_data($new_url,$con,$query,$location,$total,$jobId,$this_user_id,$group_listing,$cron_que_id,$googleJobId,$error_proxy,$function,$logged_in_username,$start_page){
    ...
    file_put_contents('fileau.log', "\n" . 'ReCrawl Job Id:-  '.$jobId. " &&  total: ". $total[0] ."\n", FILE_APPEND);

    for ($inc=$start_page; $inc <= trim($total) ; $inc=$inc+35) { 
        file_put_contents('fileau.log', "\n" . 'ReCrawl Job Id:-  '.$jobId. " &&  total: ". $total[0] ."\n", FILE_APPEND);
        $pageNum = $total/$start_page;
        ...
        $con->query("Update yellowau_home_jobrequest SET total_pages = '".trim($total)."',pages_scrape='$inc' WHERE group_listing = '$group_listing'");
        ...
        file_put_contents('fileau.log', "\n" . 'ReCrawl Job Id:-  '.$jobId.' && group listing:- '.$group_listing.'  For '. $inc . "  total: ". $total[0] ."\n", FILE_APPEND);
        $pageNum++;
        if($inc == ($total[0] - 35) || $inc === ($total[0] - 35)){
            ...
            if($check_us == '2' || $check_us === '2'){
                ...
                file_put_contents("fileau.log","\n Date: ".date('d-m-Y H;:i:s')."ReCrawl DELETE FROM cron_que where id ='$cron_que_id' \n",FILE_APPEND);
            }
    ...
    $check_us = $function->getQueData($con,$cron_que_id);
    if($check_us == '2' || $check_us === '2'){
        $yelp_status = $function->getQueIsStatus($con,'yelp_home_jobrequest',$group_listing);
        $google_status = $function->checkIsQueCompleted($con,'gogle_home_jobrequest','isStop',$group_listing);
        if(($yelp_status === 'COMPLETED' && $google_status === '1') || ($yelp_status == 'COMPLETED' && $google_status == '1')){
            $con->query("UPDATE gogle_home_jobrequest Set query_end_time = '".date('Y-m-d h:i:s')."' , status = 'COMPLETED' where id =  ".$jobId);
            $con->query(" DELETE  FROM cron_que where id = '".$cron_que_id."'");
            file_put_contents("fileau.log","\n Date: ".date('d-m-Y H;:i:s')."ReCrawl DELETE FROM cron_que where id ='$cron_que_id' \n",FILE_APPEND);
        }
    }
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
            ...
            file_put_contents('fileau.log',"\n URL: ".$url." \n",FILE_APPEND);
			$page = get_scrape_data($url,$con,$what,$where,$pages,$jobId,$this_user_id,$group_listing,$cron_que_id,$googleJobId,$error_proxy,$function,$logged_in_username_detail['username'],$start_page);
