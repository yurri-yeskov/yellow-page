	...
	while($token !== "") {
		...
		if(strpos($token, "@") !== false) {		
			...
			if(is_valid_email($email) !== true) {
				$tld = explode(".", strtolower($token));
				$tld = array_reverse($tld);
				if(in_array($tld[0], $domains)){
...
function file_get_contents_curl($url) {
	...
	curl_setopt($ch, CURLOPT_PROXYUSERPWD, 'wntdqlvl-rotate:y2jtpysnibei');
	curl_setopt($ch, CURLOPT_PROXY, 'http://p.webshare.io:80');
    //curl_setopt($ch, CURLOPT_URL, "http://api.scraperapi.com?api_key=2baa4d21f353ebd159b32689eac1434c&url=".$url);
